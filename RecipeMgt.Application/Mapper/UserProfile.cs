...
namespace RecipeMgt.Application.Mapper
{
    public class UserProfile:Profile
    {
        public UserProfile() {
            ...
            CreateMap<User, UserResponseMgtDTO>()
    .ForMember(dest => dest.TotalRecipes,
        opt => opt.MapFrom(src => src.Recipes.Count))

    .ForMember(dest => dest.TotalFollowers,
        opt => opt.MapFrom(src => src.Followers.Count))

    .ForMember(dest => dest.UserStatus,
        opt => opt.MapFrom(src =>
            src.DeleteAt != null ? UserStatus.Deleted :
            src.IsBanned ? UserStatus.Banned :
            !src.IsActived ? UserStatus.Deactivated :
                                   UserStatus.Active
        ));
